...
(function (global, factory) {
    ...
    const createDateParser = ({ config = defaults, l10n = english }) => (date, givenFormat, timeless) => {
        if (date !== 0 && !date)
            return undefined;
        let parsedDate;
            ...
            if (datestr === "today") {
                ...
                for (let i = 0, matchIndex = 0, regexStr = ""; i < format.length; i++) {
                    ...
                    if (tokenRegex[token] && !escaped) {
                        ...
                        regexStr += ".";
                    ops.forEach(({ fn, val }) => (parsedDate = fn(parsedDate, val, l10n) || parsedDate));
                }
    ...
    function FlatpickrInstance(element, instanceConfig) {
        ...
        function bindEvents() {
            if (!self.isMobile && self.config.wrap) {
        ...
        function buildMonth() {
            ...
            const yearElement = yearInput.childNodes[0];
      ...
      `;
        ...
        function onMouseOver(elem) {
            if (self.selectedDates.length !== 1 ||
                ...
                return;
            const hoverDate = elem.dateObj.getTime(), initialDate = self.parseDate(self.selectedDates[0], undefined, true).getTime(), rangeStartDate = Math.min(hoverDate, self.selectedDates[0].getTime()), rangeEndDate = Math.max(hoverDate, self.selectedDates[0].getTime());
        ...
        function open(e, positionElement = self._input) {
        ...
        function updateValue(triggerChange = true) {
            ...
            const joinChar = self.config.mode !== "range"
